...
titleQuestionButtons.forEach(titleQuestionButton => {
  titleQuestionButton.addEventListener('click', () => {
    ...
    if (isClick) {
      if (windowWidth < 480) {
        sliderBtn.style.background =
          '#6C5FBC url("../images/menu/minus.png") center /15px no-repeat';
        isClick = false;
      } else {
        sliderBtn.style.background =
          '#6C5FBC url("../images/menu/minus.png") center no-repeat';
        isClick = false;
      }
    } else {
      if (windowWidth < 480) {
        sliderBtn.style.background =
          '#6C5FBC url("../images/menu/plus.png") center /15px no-repeat';
        isClick = true;
      } else {
        sliderBtn.style.background =
          '#6C5FBC url("../images/menu/plus.png") center no-repeat';
        isClick = true;
      }
    }
  });
